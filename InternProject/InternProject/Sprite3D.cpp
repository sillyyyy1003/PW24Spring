...
void Sprite3D::GenerateMatrix(ConstBuffer& cb)
{
	//カメラの行列を単位行列
...
void Sprite3D::SetPos(float x, float y, float z)
{
	mPos.x = x;
	mPos.y = y;
	mPos.z = z;
...
void Sprite3D::SetRotate(float x, float y, float z)
{
	mRotation.x = x;
	mRotation.y = y;
	mRotation.z = z;
...
void Sprite3D::InitCanvas(DirectX::XMFLOAT2 _size, DirectX::XMINT2 _split, ID3D11ShaderResourceView* _assets, Anime::AnimePattern _pattern, ModelSet::VerticalAlign _align)
{
	//Create Model
...
void Sprite3D::SetMaterialDiffuse(Color sRGB)
{
